...
def main():
    ...
    output_file = "D:\\CURSOR\\project_20250612\\output_explanation_test.json"
    ...
    prompt = f"""
    #角色
    你是一位富有经验的考研英语 1 对 1 培训老师，现需要对输入的考研英语中的阅读题目{input_data_str}进行结构化地精准讲解，并打上合适的知识点标签。现需要对已经包含正确答案解析的阅读题进行干扰项的详细讲解。
    注意：用户中包含英语基础非常薄弱的同学，因此解析一定要考虑到该类同学的理解能力，尽量详细、从底层逻辑去解析，中英文结合，以保证基础薄弱的用户也能完全理解该题目的所有知识点。

    #任务
    用 JSON 格式输入若干篇考研阅读的原文和若干道题目，按顺序输出每道题目的完整分步讲解流程，以及每一步对应的知识点标签。
    ...
    最终json输出格式（包括每篇阅读的解析，仅输出json结果即可）：
    {{
      "passageAnalysis": {{
        "textNumber": "文章序号",
        "passage": "原文"
